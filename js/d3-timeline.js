  ...
  d3.timeline = function(myflag) {
	...
	if (myflag !=2){
      ...
      g.each(function(d, i) {
		...
		//console.log("hello!!!!!"+margin.top + (itemHeight + itemMargin) * yAxisMapping[index])
          g.selectAll("svg").data(data).enter()
            ...
            .attr("width", function (d, i) {
				if (myflag == 1) {
				return (d.current - d.starting_time) * scaleFactor;
				}
             else{ 
			...
			.attr("stroke-width",function(){				
				if (myflag == 0) {return 3;}
				else if (myflag == 1) { return "none";}
			})
			.style("fill",function(d, i){
				if (myflag == 0) { return "#E6E6E6";}
				else if (myflag == 1 || myflag ==2) {
					  if( colorPropertyName ){ 
						return colorCycle( datum[colorPropertyName] ) 
					  } 
					  return colorCycle(index); 					
